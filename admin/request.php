...
<div id="mySidenav" class="sidenav">
    ...
    <div id="main">
        ...
        <div class="container scroll">
            ...
            <h3 style="text-align: center; font-size:30px;"><b>Request of Book</b></h3>
            <?php
            if(isset($_SESSION['login_admin']))
            {
                $sql = "SELECT student.username,roll,books.b_id,name,authors,edition,status FROM student INNER JOIN issue_book ON student.username = issue_book.username INNER JOIN books ON issue_book.b_id = books.b_id WHERE issue_book.approve = ' '";
            ...
            if(isset($_POST['submit']))
            {
                ...
                // echo "<pre>"; print_r($_SESSION['name']);
                // echo "<pre>"; print_r($_SESSION['b_id']);
                // exit();
                $sql = "SELECT student.username,roll,books.b_id,name,authors,edition,status FROM student INNER JOIN issue_book ON student.username = issue_book.username INNER JOIN books ON issue_book.b_id = books.b_id WHERE issue_book.approve = ' '";
                $res = mysqli_query($db, $sql);
                while($row = mysqli_fetch_assoc($res))
                {
                    ...
                    // echo "<pre>"; print_r($_POST['b_id']);
                    // echo "<pre>"; print_r($row['b_id']);
                    // exit();
                    if($_POST['username'] == $row['username'] && $_POST['b_id'] == $row['b_id'])
                    {
                    ?>
                    <script>
                ...
                echo "<br>";
                echo "<h2 style='text-align: center;'>Please Enter correct username and book id.</h2>";
            } 
            ?>
        </div>
    </div>
